<?php 
	trait ProductModel{
		...
		public function modelDoEdit($id){
			...
			$salesproduct = isset($_POST["salesproduct"])?1:0;
			//---
			//lay bien ket noi
			$conn = Connection::getInstance();
			//chuan bi truy van
			$query = $conn->prepare("update tbl_product set name=:name,category_id=:category_id,description=:description,content=:content,hotproduct=:hotproduct,salesproduct=:salesproduct,price=:price where id=:id");
			//thuc thi truy van
			$query->execute(array("id"=>$id,"name"=>$name,"category_id"=>$category_id,"description"=>$description,"content"=>$content,"hotproduct"=>$hotproduct,"salesproduct"=>$salesproduct,"price"=>$price));
			//---
		...
		public function modelDoAdd(){
			...
			$conn = Connection::getInstance();
			//chuan bi truy van
			$query = $conn->prepare("insert into tbl_product set name=:name,category_id=:category_id,description=:description,content=:content,hotproduct=:hotproduct,salesproduct=:salesproduct,img=:img,price=:price");
			//thuc thi truy van
			$query->execute(array("img"=>$img,"name"=>$name,"category_id"=>$category_id,"description"=>$description,"content"=>$content,"hotproduct"=>$hotproduct,"salesproduct"=>$salesproduct,"price"=>$price));
			//---
